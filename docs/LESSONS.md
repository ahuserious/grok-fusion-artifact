...
- Record requested model, actual provider model, and native served-build telemetry separately.
- A one-turn native agent that starts tools may never reach its verdict turn; align tool policy and turn budget.
- Keep cancelled and failed attempts in the spend/evidence record.
- A missing key or DNS failure is transport/preflight evidence, not a model-quality score.
